...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template name="writeCssFile"> 
        <xsl:result-document href="{testng:absolutePath('style.css')}" format="text"> 
            ...
            <![CDATA[ 
            body { font-family: Arial, sans-serif; font-size: 12px; padding: 10px; margin: 0px; backgroundcolor: white; } 
            ...
            .testMethodStatusFAIL a, .testMethodStatusPASS a, .testMethodStatusSKIP a { color:navy; textdecoration: none; cursor: pointer; } 
    ...
    <xsl:function name="testng:suiteStateClass"> 
        ...
        <xsl:value-of select="if (count($testCaseElements/class/test-method[(@status='FAIL') and (not(@isconfig))]) > 0) then 'suiteStatusFail' else 'suiteStatusPass'"/> 
    </xsl:function> 
    ...
    <xsl:template match="/testng-results"> 
        ...
        <xsl:result-document href="{testng:absolutePath('overview-chart.svg')}" format="xml"> 
            <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" width="{$chartWidth}" height="{$chartHeight}" 
                ...
                .values { fill:black; stroke:none; text-anchor:middle; fontsize:12px; font-weight:bold; } 
                  ]]> 
                    </style> 
                </defs> 
                            ...
                            <xsl:for-each select="$testCaseElements"> 
                                <tr style="background-color: #f5f5f5; font-size: 12px;"> 
                                    ...
                                    <td align="center" style="font-weight: bold;"> 
                                        <xsl:value-of 
                                                select="if (testng:testCaseMethodsCount(., '*') > 0) then formatnumber(testng:testCaseMethodsCount(., 'PASS') div testng:testCaseMethodsCount(., '*'), '###%') else '100%'"/> 
                                    </td> 
        ...
        <xsl:for-each select="$testCases"> 
            ...
            <div class="testCaseLink" 
                ...
                <div class="{if (count(./class/test-method[@status='FAIL']) > 0) 
                                then 'testCaseFail' 
                                else if ((count(./class/test-method[@status='FAIL']) = 0) and (count(./class/testmethod[@status='PASS']) > 0)) 
                                    then 'testCasePass' 
                                    else 'testCaseSkip'}"> 
                </div> 
                <xsl:value-of select="@name"/> 
